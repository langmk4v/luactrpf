...
class Parser {
 ...
 public:
  ...
  bool eat_open_of(TokBrackets b) {
    return cur->is(b, true) && (next(), true);
  }
  ...
  Expr* p_factor() {
    ...
    Logger::Emit(Utils::Format("cur=%p,kind=%d,str=",
          cur,static_cast<int>(cur->kind)) + string(cur->get_strview()));
  ...
  auto p_ifs(Token* iftok) -> ast::If* {
    if(auto if_cond = p_expr()){
      ...
      source->add_error(
        Error(iftok,"expected expression after this token."));
    }
    return nullptr;
  }
  ...
  auto p_stmt() -> Stmt* {
    ...
    if (auto x = p_expr()){
      ...
      source->add_error(
        Error(cur, "expected statement or expression."));
      return nullptr;
    }
  }
